...
#### 1.10 Machines by kind — **done**

`byKind` sums per-item `machines` that share `recipe.machine`. Ore (`null`) is skipped. Red 1/s steel: furnaces = copper plate + iron plate (**4.8**). Assemblers = pack + gear.
